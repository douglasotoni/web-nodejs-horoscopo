...
// Frases específicas por fase da lua (máx. 30 por fase) — espelhado de lib/generator.ts
...
export function generateWeeklyPredictionClient(context: Omit<GeneratorContext, 'weekday'>): {
  ...
  sentences.push(advice
    ...
    // Mensagem final (máx. 30 variações) — espelhado de lib/generator.ts
    const finalMessages = [
    ...
    // Frase extra opcional (máx. 30 variações) — espelhado de lib/generator.ts
    if (seedNums[1] % 3 !== 0) {
      const extraMessages = [
